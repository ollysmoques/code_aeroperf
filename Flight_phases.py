...
def FLIGHT_PHASES(h_cruise, h_airport, dT_isa, VY, V_cruise_CAS, TO_weight, Weight_Reserve):
    ...
    print(f"Total time         : {temps_total_min:.2f} min ({temps_total_min/60:.2f} h)")
    print(f"Total distance     : {distance_total_NM:.2f} NM")
    print("--------------------------------------")
    ...
    print(f"  Start descent    : {Weight_top_descent:.2f} lb")
    print(f"  Final (1000 ft)      : {weight_at_1000ft:.2f} lb")
    print(f"  Touchdown : {w_td:.2f} lb")
